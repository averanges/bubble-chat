...
import { searchFriend } from '../../services/userService'
import { loggedUser } from '../../slices/authSlice'
import { foundFriends, openAddFriendPopup } from '../../slices/userSlice'
import { SearchButton } from '../buttons/Buttons'
import SearchProfile from '../sub-components/SearchProfile'
import { openWelcomeUserPopup, switcherTab } from '../../slices/popupSlice'

const AddFriendPopUp = () => {
    const dispatch = useDispatch()
    const user = useSelector(loggedUser)
    const searchedFriends = useSelector(foundFriends)
    ...
    useEffect (() => {
      dispatch(searchFriend({user, searchFriendValue}))
    },[searchFriendValue, searchByName])
    const listOfSearchedFriends = (searchedFriends && searchedFriends.length) && searchedFriends
    .map(el => <SearchProfile props={el} key={el._id}/>)
  return (
        ...
        <div className='w-80 h-96 bg-white relative p-5 flex flex-col gap-7 dark:bg-side-dark dark:text-white'>
          ...
          >
           ...
           <div className='flex w-full gap-2'>
            ...
            <span onClick={() => setSearchByName('name')}
            className={`text-xl cursor-pointer relative ${searchByName === 'name' && 'before:block before:absolute before:-inset-1 before:-skew-y-3 before:opacity-50 before:bg-teal-500'}`}>UserName</span>
            <span className='text-xl'>/</span>
            <span onClick={() => setSearchByName('email')}
            className={`text-xl cursor-pointer relative ${searchByName === 'email' && 'before:block before:absolute before:-inset-1 before:-skew-y-3 before:opacity-50 before:bg-teal-500'}`}>Email</span>
           </div>
           <div className='relative h-10 min-h-[2.5rem] w-full '>
                <SearchButton/>
            ...
            {listOfSearchedFriends ? listOfSearchedFriends : null}
            </div>
        </div>
    </div>
  )
